<template>
    <div>
        <a-table :dataSource="data" :columns="columns" :scroll="{ x: 1000}">
            ...
            <a-drawer title="详情" width=720 placement="right" :closable="false" @close="onClose" :visible="visible"
                ...
                <div>
                    <a-collapse accordion>
                        ...
                        <a-collapse-panel header="用户拥有账户" key="2" :disabled='false'>
                            <a-table :columns="accountColumns" :dataSource="accountData">
                                <template slot="operation" slot-scope="text, record">
                        ...
                        if (result.status == 200) {
                             for(let i=0;i<this.accountData.length;i++){
                                 if(this.accountData[i].account==this.delAccount){
                                     this.accountData[i].accountStatus='已经删除';
                                     break;
                                 }
                             }
                             this.delAccount=0;

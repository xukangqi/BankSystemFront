<template>
	<a-form @submit="handleSubmit" :autoFormCreate="(form)=>{this.form = form}">
		<a-form-item label='申请人' :labelCol="{ span: 5 }" :wrapperCol="{ span: 12 }" fieldDecoratorId="name"
	 	...
	 	<a-form-item label='交易时间' :labelCol="{ span: 5 }" :wrapperCol="{ span: 12 }" fieldDecoratorId="transfer_date">
	 		{{CurrentTime}}
	 		<a-input type="hidden" :defaultValue="CurrentTime" />
	 	</a-form-item>
		...
		methods: {
			...
			handleSubmit(e) {
		        ...
		        this.form.validateFields((err, values) => {
		          if (!err) {
		          	values.transfer_date = this.CurrentTime;
